...
| `RADIANCE_EMBED_HOST` | `0` | Keep the target's `embed_tokens` (248320 x 5120 bf16, **2.37 GiB**) in pinned host memory and let the GPU read it over PCIe through a UVA view, handing that VRAM to the KV cache (~60k tokens at TP=1). A forward pass reads one 10 KB row per token -- ~16 rows per decode step, 26 MB per 2560-token prefill chunk -- so the table can leave VRAM without being streamed back; `lm_head`, the same size but multiplied in full every step, cannot. Refused, with the reason logged and the table left in VRAM, if it is tied to `lm_head`, quantized, or does not read back bit-exact through the view; never falls back to vLLM's re-upload-per-forward offload. The startup line `[radiance] embed-host:` reports the VRAM freed and the measured gather cost. KV pins measured with it on are keyed `<spec>+eh` in `kv-profiles.tsv` and are never read with it off. Measured 2026-09-22, one R9700 at TP=1 (MAXSEQS=4, CHUNK=2560, dflash SPEC=7, profiled KV): model 20.0 -> **17.63 GiB**, KV pool 136,441 -> **199,920 tokens** at MAXLEN=130240 and **205,105** at 196608, so the served ceiling goes 130,240 -> **196,608**. Cost, from the startup line: bulk readback 12.1 GB/s, gather 16 rows 9.4 -> 21.6 us (+12 us per decode step, 0.03% of one) and 2560 rows 44.6 -> 1811.8 us (+1.8 ms per prefill chunk, 0.2%). End to end: cold prefill at 124K **2,283 t/s** against 2,280 before the change, 2,008 t/s cold at 170K; text, vision and a 170K prefix-cache hit all clean. A/B against the flag off at
1K/5K/10K/50K/128K: step time identical to within 0.1 ms and every prefill row fractionally ahead
WITH the flag on, so the cost is bounded under ~1% rather than measured; 150K/180K have no
flag-off counterpart (63.90 s / 81.46 s TTFT) |
